...
abstract class Model {
	...
	function __construct() {
		$this->logFile = APP_PATH.'/log/sql/'.date('Y-m-d').'.log';
		self::$conn = Pool::getInstance('mysql');
	}
	 ...
	 */
	final public function Insert($map = array(), $ignore = 0) {
		if (!$map || !is_array($map)) {
			return FALSE;
		} else {
			$fields = $values = array();
		...
		if (self::$conn->errorCode() != $this->successCode) {
			...
			if (ENV == 'DEV') {
				...
				*/
				...
				$e['app']      = APP_NAME;
				$e['ip']       = getClientIP();
				$e['statement']= $this->sql;
				$e['error']    = $error[2];
				$e['addTime']  = date('Y-m-d H:i:s');
				...
				$sql = "INSERT INTO `error` ($fieldString) VALUES ($valueString)";
				self::$conn->query($sql);
